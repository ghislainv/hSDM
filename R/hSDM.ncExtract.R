hSDM.ncExtract <- function(files,what=c("eval","coef","autocor","predictions","envdata")){
  ...
  foreach(f=files,.combine=rbind.data.frame)%do%{
    ...
    meta=c(species,modeltype,modelname,suitability,observability)
    ...
    if(what=="eval"){
      ...
      return(cbind.data.frame(meta,evaluation))
    }
    if(what=="coef"){
