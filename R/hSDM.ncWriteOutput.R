#' @title hSDM.ncWriteOutput
#' @description Write hSDM model output to netCDF file for archival and model comparison
#'
#' @author Adam M. Wilson
#' @param results \code{hSDM.*} model output object
#' @param file name of netcdf file to write
#' @param overwrite logical indicating whether to overwrite existing output file.
#' @param autocor logical indicating whether to calculate spatial autocorrelation of model predictions
#' @param keepall logical indicating whether to save all posterior samples (e.g. from `save.rho` or `save.p`) or calculate quantile summaries.
#' @param meta list of any number of arbitrary named character strings to append as attributes in the output file.  These could include, for example, \code{meta=list(species="Protea puncata",region="South Africa",model="Full")}.  
#' @param verbose \code{logical} indicating whether to report on progress
#' @return A netCDF file is written with model output summaries.
#' @examples
#' 
##
####################################################################

hSDM.ncWriteOutput<-function(results,file,overwrite=T,
                             autocor=F,keepall=F,meta=NULL,verbose=T){
  ...
  ## combine multiple chains to a mcmc.list
  if(length(results<2)) stop("Number of chains must be >1 to compute convergence metrics")
  parameters_list=mcmc.list(lapply(results,FUN=function(x) x$mcmc))
  
  ## assess convergence for each parameter in mcmc
  ...
  colnames(c3)=c("Heidel.Stationarity","Heidel.Start",
                 "Heidel.P","Heidel.HalfwidthTest","Heidel.Mean",
                 "Heidel.Halfwidth")
  ...
  # rasterfromXYZ
  predr=SpatialPixelsDataFrame(predr,tolerance=0.005,
                               data=data.frame(predr[,c("pred","cell")]))
  fullgrid(predr)=T
  predr=stack(predr)
  ...
  ## Set dimentions
  d_lat=ncdim_def("lat",units="degrees_north",
                  vals=sort(unique(coordinates(predr)[,2]),decreasing=F),longname="latitude")
  d_lon=ncdim_def("lon",units="degrees_east",
                  vals=sort(unique(coordinates(predr)[,1])),longname="longitude")
  d_iter=ncdim_def("iter",units="iterations",
                   longname="Posterior Iterations",
                   vals=1:nrow(results[[1]]$mcmc),unlim=TRUE)
  
  v_var_mean=ncvar_def("p",units="probability",
                       dim=list(d_lon,d_lat),missval=-999,
                       longname="p(occurrence|data)",
                       compression=comp,prec="integer")
  v_var_cell=ncvar_def("cell",units="cell",
                       dim=list(d_lon,d_lat),missval=-999,
                       longname="Unique gridcell ID",
                       compression=comp,prec="integer")
  ...
  d_params1=ncdim_def("parameters1",units="",
                      create_dimvar=F,vals=1:ncol(parameters))  
  d_params2=ncdim_def("parameters2",units="",
                      create_dimvar=F,vals=1:nrow(parameters)) 
  #evaluation summaries
  d_eval1=ncdim_def("evaluation1",units="",
                    create_dimvar=F,vals=1:ncol(evaluation)) 
  d_eval2=ncdim_def("evaluation2",units="",
                    create_dimvar=F,vals=1:nrow(evaluation)) 
  
  ## Define variables
  v_var_parameters=ncvar_def("parameters",units="values",
                             dim=list(d_params1,d_params2),missval=-999,
                             longname="Posterior Parameter Values",compression=comp)
  v_var_evaluate=ncvar_def("evaluation",units="values",
                           dim=list(d_eval1,d_eval2),missval=-999,
  ...
  if(!keepall&autocor) nc_create(file,
                                 vars=list(v_var_mean,v_var_cell,
                                           v_var_parameters,v_var_evaluate,v_var_ac),verbose=F)   #save every iteration
  if(!keepall&!autocor) nc_create(file,
                                  vars=list(v_var_mean,v_var_cell,
                                            v_var_parameters,v_var_evaluate),verbose=F)   
  ...
  ## Global Attributes
  ncatt_put(nc,varid=0, 
            "Conventions","Cf-1.4",
            prec="character")
  ncatt_put(nc,varid=0, "title",
            paste0("Predicted p(occurrence)"),
            prec="character")
  ncatt_put(nc,varid=0, "modeltype",
            as.character(results[[1]]$meta$modeltype),
            prec="character")
  ncatt_put(nc,varid=0, "suitability",
            as.character(results[[1]]$meta$suitability),
            prec="character")
  ncatt_put(nc,varid=0, "observability",
            as.character(results[[1]]$meta$observability),
            prec="character")
  ncatt_put(nc,varid=0, "date",
            as.character(today),prec="character")
  ## add any additional metadata listed in the meta object
  if(!is.null(meta))
    for(i in 1:length(meta))  #loop over elements in meta list
      ncatt_put(nc,varid=0, 
                as.character(names(meta)[i]),
                as.character(unlist(meta[i])),
                prec="character")
