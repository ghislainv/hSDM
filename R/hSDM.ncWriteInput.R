hSDM.ncWriteInput<-function(envdata,points,ncfile,meta=NULL,overwrite=F,verbose=T){
  ...
  d_lat=ncdim_def("lat",units="degrees_north",
                  vals=unique(coordinates(envdata)[,2]),longname="latitude")
  d_lon=ncdim_def("lon",units="degrees_east",
                  vals=unique(coordinates(envdata)[,1]),longname="longitude")
  d_var=ncdim_def("variable",units="",create_dimvar=F,vals=1:nlayers(envdata))  

    ## Define variables
  comp=9  #define compression level: 9 is the highest
  v_var_var=ncvar_def("envdata",units="standardized",
                      dim=list(d_lon,d_lat,d_var),missval=-999,
                       longname="environmental covariates",
                      compress=comp,prec="float")
  v_var_obs=ncvar_def("obs",units="presence",
                      dim=list(d_lon,d_lat),missval=-999,
                      longname="Observations",
                      compress=comp,prec="integer")
  v_var_trials=ncvar_def("trials",units="trial",
                     dim=list(d_lon,d_lat),missval=-999,
  ...
  ## set up nc file
  nc_create(ncfile,
            vars=list(v_var_var,v_var_obs,v_var_trials,v_var_cell),
            verbose=F) 
  ...
  ## Add data
  lapply(1:nlayers(senv), function(i) 
    ncvar_put(nc,"envdata",
              vals=t(raster::as.matrix(senv[[i]])),
  ...
  ncvar_put(nc,"obs",
            vals=t(raster::as.matrix(presences)),start=c(1,1),c(-1,-1),verb=F)
  ncvar_put(nc,"trials",
            vals=t(raster::as.matrix(trials)),start=c(1,1),c(-1,-1),verb=F)
  ncvar_put(nc,"cell",
            vals=t(raster::as.matrix(cell)),start=c(1,1),c(-1,-1),verb=F)
  ...
  if(!is.null(meta))
    for(i in 1:length(meta)) 
      ncatt_put(nc,varid=0, names(meta)[i],unlist(meta[i]),prec="character")
