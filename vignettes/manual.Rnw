...
# Number of spatial entity (raster cells)
ncells <- ncell(alt)
# Distance matrix
distance <- as.matrix(dist(coords))
# Generate spatial random effects
delta <- 0.05
rho <- rmvn(1,mu=rep(0, ncells),V=exp(-delta * distance),seed)
rho <- rho-mean(rho) # Centering rhos on zero
rho.rast <- rasterFromXYZ(xyz=cbind(coords,rho))
plot(c(1:100),exp(-delta*c(1:100)),type="l",
     xlab="Distance",ylab="Correlation")
plot(rho.rast)
# Probability of presence
theta.cells <- inv.logit(X %*% beta.target + rho)
theta <- rasterFromXYZ(cbind(coords,theta.cells))
plot(theta,main="Initial probabilities",col=colRP(nb),breaks=brks,
     axis.args=arg,zlim=c(0,1))
@ 

\begin{figure} 
  \begin{tabular}{cc}
    \includegraphics[width=8cm]{figures/binom-iCAR-spatial-effects1.pdf} &
    \includegraphics[width=8cm]{figures/binom-iCAR-spatial-effects2.pdf} \\
  \end{tabular}
  \caption{\textbf{Spatial random effects}}
  \label{fig:binom-iCAR-spatial-effects}
\end{figure}
...
plot(rho.pred.rast,main="Predictions")

theta.pred <- mod.hSDM.binomial.iCAR$prob.p.pred
theta.pred.rast <- rasterFromXYZ(cbind(coords,theta.pred))
plot(theta.pred.rast,main="Predictions",col=colRP(nb),breaks=brks,
     axis.args=arg,zlim=c(0,1))
@ 

\begin{figure} 
  \centering \includegraphics[width=8cm]{figures/binom-iCAR-results.pdf}
  \caption{\textbf{Spatial random effect predictions}}
