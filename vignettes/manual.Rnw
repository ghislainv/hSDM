...
## require(fields)
## col.prob <- colorRampPalette(c("transparent","blue","green","yellow","orange","red","brown"))
...
theta.obs <- inv.logit(logit.theta.obs)
set.seed(seed)
Z <- rbinom(nobs,1,theta.obs)

# Observation process (detectability)
logit.delta.obs <- W %*% gamma.target
delta.obs <- inv.logit(logit.delta.obs)
set.seed(seed)
Y <- rbinom(nobs,5,delta.obs*Z)

# Data-set
trials <- rep(5,nobs)
data.obs <- data.frame(Y,trials,alt=X.obs[,2],alt2=X.obs[,3],w1,w2)
@ 
...
<<inference-siteocc>>=
mod.hSDM.siteocc <- hSDM.ZIB(presences=data.obs$Y, ## Change here !!
                                 trials=data.obs$trials,
                                 suitability=~alt+alt2,
                                 observability=~w1+w2,
                                 data=data.obs,
...
theta.pred.mean <- raster(theta)
# Attribute predicted values to raster cells
theta.pred.mean[] <- mod.hSDM.siteocc$prob.p.pred
