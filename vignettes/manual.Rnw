...
## require(fields)
## col.prob <- colorRampPalette(c("transparent","blue","green",
##                                "yellow","orange","red","brown"))
## plot(theta,col=col.prob(255))
...
# Load dismo library
require(dismo) # For randomPoints() function
...
X <- cbind(rep(1,nobspt),alt.obs,alt.obs^2)
logit.theta.obs <- X %*% beta.target
...
data.obs.df <- data.frame(Y,trials,alt=X[,2],alt2=X[,3])
...
logit.theta.obspt <- X %*% beta.target
theta.obspt <- inv.logit(logit.theta.obspt)
set.seed(seed)
Z <- rbinom(nobspt,1,theta.obspt)

# Observation process (detectability)
logit.delta.obs <- W %*% gamma.target
delta.obs <- inv.logit(logit.delta.obs)
set.seed(seed)
# Y <- rbinom(nobs,1,delta.obs*Z[obspt])
Y <- rbinom(nobs,1,0.8*Z[obspt])

# Spatial entity (raster cells) associated to each observation
cell.obspt <- extract(alt,obs,cellnumbers=TRUE)[,"cells"]
cell.obs <- cell.obspt[obspt]
# cell.obs <- as.numeric(as.factor(cell.obs)) # from 1 to ncell, not necessary

# Data-sets
data.obs <- data.frame(Y,w1,w2,cell=cell.obs)
data.suit <- data.frame(alt=X[,2],alt2=X[,3])
@ 
...
theta.pred.mean <- raster(theta)
# Attribute predicted values to raster cells
theta.pred.mean[] <- mod.hSDM.siteocc$theta.pred
...
<<results="markup">>=
#== glm results for comparison
data.obs.glm <- data.frame(Y,alt=X[obspt,2],alt2=X[obspt,3])
mod.glm <- glm(Y~alt+alt2,family="binomial",data=data.obs.glm)
summary(mod.glm)
@ 
...
# Plot the predicted probability of presence
plot(theta.pred.glm,col=rev(heat.colors(255)))
@

<<pred-obs-siteocc-glm>>=
# Comparing predictions to initial values
plot(theta[],theta.pred.glm[],cex.lab=1.4)
abline(a=0,b=1,col="red",lwd=2)
@ 
