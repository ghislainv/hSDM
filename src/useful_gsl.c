...
double integer(const gsl_rng *r, double a, double b)                             
{
  double u,x;
  double *wl,*wlc;
  int i;
...
/* the following function returns a random number from TG^+(a,b,1)            */  
/* a=shape parameter, b=rate parameter                                        */ 
double inter_le(const gsl_rng *r, double a, double b)
{
  ...
  else
  {
    if (a<b) 
    {
      ...
      while (test == 0)
      {
        x=integer(r, floor(a), b*floor(a)/a);
